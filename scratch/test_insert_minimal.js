async function test() {
  ...
  const testFacture = {
    ...
    montantttc: 1200,
    devise: "MAD",
    motif: "Test lowercase",
    lignes: [{ description: "test", quantite: 1, prixUnitaire: 1000, totalLigne: 1000 }]
  };

  console.log("Attempting lowercase minimal insertion...");
  ...
  if (res.ok) {
    ...
    await fetch(`${sbUrl}/rest/v1/factures?id=eq.test_temp_id_lowercase`, {
      method: 'DELETE',
      headers: {
        'apikey': sbKey,
        'Authorization': `Bearer ${sbKey}`
